...
다중 카메라 관제 월 + 클래스별 성능 차트 + 포커스 채널(채널 선택·화재 위험도) 구성.
"""
...
st.divider()
st.markdown("#### 🔍 포커스 채널 (채널 선택 · 정밀 분석)")


@st.fragment
def focus():
    names = [x["name"] for x in feeds]
    sel = st.radio("채널 선택", names, horizontal=True, label_visibility="collapsed")
    f = next(x for x in feeds if x["name"] == sel)
    ...
    with cL:
        ...
        st.markdown(f'<div class="alert {cls_}">탐지 상태: {lv} — {msg}</div>', unsafe_allow_html=True)
    with cR:
